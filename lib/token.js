...
const parse = tokens => {
  ...
  const interpretToken = token => {
    let sym;
    if(token.type === 'operator') sym = Object.create(symbols[token.value]);
    else sym = Object.create(symbols[token.type]);
    sym.type = token.type;
...
console.log('***HELLO***', evaluate(parse(tokenize('2*7+(3)'))));
